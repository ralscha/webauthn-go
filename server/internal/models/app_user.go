...
type appUserR struct {
	AppCredentials AppCredentialSlice `boil:"AppCredentials" json:"AppCredentials" toml:"AppCredentials" yaml:"AppCredentials"`
}
...
func (r *appUserR) GetAppCredentials() AppCredentialSlice {
	if r == nil {
		return nil
	}
	return r.AppCredentials
}
...
// AppCredentials retrieves all the app_credential's AppCredentials with an executor.
func (o *AppUser) AppCredentials(mods ...qm.QueryMod) appCredentialQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"app_credentials\".\"app_user_id\"=?", o.ID),
	)

	return AppCredentials(queryMods...)
}

// LoadAppCredentials allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (appUserL) LoadAppCredentials(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAppUser interface{}, mods queries.Applicator) error {
	var slice []*AppUser
	var object *AppUser
	...
	query := NewQuery(
		qm.From(`app_credentials`),
		qm.WhereIn(`app_credentials.app_user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load app_credentials")
	}

	var resultSlice []*AppCredential
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice app_credentials")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on app_credentials")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for app_credentials")
	}

	if singular {
		object.R.AppCredentials = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &appCredentialR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.AppUserID {
				local.R.AppCredentials = append(local.R.AppCredentials, foreign)
				if foreign.R == nil {
					foreign.R = &appCredentialR{}
				}
...
// AddAppCredentials adds the given related objects to the existing relationships
// of the app_user, optionally inserting them as new records.
// Appends related to o.R.AppCredentials.
// Sets related.R.AppUser appropriately.
func (o *AppUser) AddAppCredentials(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*AppCredential) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.ID, rel.AppUserID}
	...
	if o.R == nil {
		o.R = &appUserR{
			AppCredentials: related,
		}
	} else {
		o.R.AppCredentials = append(o.R.AppCredentials, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &appCredentialR{
				AppUser: o,
			}
		} else {
			rel.R.AppUser = o
