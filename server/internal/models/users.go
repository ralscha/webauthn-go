...
var UserTableColumns = struct {
	...
	RegistrationStart: "users.registration_start",
...
var UserWhere = struct {
	...
	RegistrationStart: whereHelpernull_Time{field: "\"users\".\"registration_start\""},
...
type userR struct {
	Credentials CredentialSlice `boil:"Credentials" json:"Credentials" toml:"Credentials" yaml:"Credentials"`
}
...
func (r *userR) GetCredentials() CredentialSlice {
	if r == nil {
		return nil
	}
...
var (
	userAllColumns            = []string{"id", "username", "registration_start", "created_at"}
	userColumnsWithoutDefault = []string{"username"}
	userColumnsWithDefault    = []string{"id", "registration_start", "created_at"}
...
// Credentials retrieves all the credential's Credentials with an executor.
func (o *User) Credentials(mods ...qm.QueryMod) credentialQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"credentials\".\"user_id\"=?", o.ID),
	)

	return Credentials(queryMods...)
}

// LoadCredentials allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCredentials(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`credentials`),
		qm.WhereIn(`credentials.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load credentials")
	}

	var resultSlice []*Credential
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice credentials")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on credentials")
	}
	...
	if singular {
		object.R.Credentials = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Credentials = append(local.R.Credentials, foreign)
				if foreign.R == nil {
					foreign.R = &credentialR{}
				}
...
// AddCredentials adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Credentials.
// Sets related.R.User appropriately.
func (o *User) AddCredentials(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Credential) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.CredID}
	...
	if o.R == nil {
		...
		o.R.Credentials = append(o.R.Credentials, related...)
	}
